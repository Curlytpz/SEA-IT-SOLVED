...
export default function CalibrationImageLightbox({ inspection, onClose }) {
  ...
  return <Dialog open={Boolean(inspection)} onOpenChange={open => { if (!open) onClose(); }}>
    <DialogContent showCloseButton={false} className="flex h-[calc(100dvh-.5rem)] max-h-[calc(100dvh-.5rem)] max-w-[calc(100%-0.5rem)] flex-col gap-0 overflow-hidden p-0 sm:h-[calc(100dvh-2rem)] sm:max-h-[calc(100dvh-2rem)] sm:max-w-[min(96vw,1600px)]">
      <header className="flex shrink-0 items-start justify-between gap-3 border-b border-border px-3 py-3 dark:border-border sm:px-5">
        ...
        <button type="button" onClick={onClose} aria-label="Close full-size calibration viewer" className="grid h-11 w-11 shrink-0 place-items-center rounded-lg text-muted-foreground hover:bg-destructive-subtle hover:text-destructive-subtle-foreground dark:text-muted-foreground dark:hover:bg-destructive-subtle">
          <X size={20}/>
        </button>
      </header>
      <div className="flex min-h-0 flex-1 touch-auto items-center justify-center overflow-auto bg-slate-950 p-1 sm:p-4">
        {inspection && <div className="relative inline-block max-h-full max-w-full leading-none">
          ...
          {points.map((point, index) => <span key={point.id || index} aria-hidden="true" className="pointer-events-none absolute h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-primary bg-card shadow" style={{ left: `${point.x * 100}%`, top: `${point.y * 100}%` }}/>) }
        </div>}
      </div>
    </DialogContent>
  </Dialog>;
