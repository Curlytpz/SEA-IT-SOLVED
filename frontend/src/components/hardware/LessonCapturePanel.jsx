...
export default function LessonCapturePanel({lesson,onReadinessChange}){
  ...
  const calibrationClass=calibrationStatus==='CALIBRATED'?'text-success-subtle-foreground dark:text-success-subtle-foreground':calibrationStatus==='INVALID'?'text-destructive-subtle-foreground dark:text-destructive-subtle-foreground':calibrationStatus==='LOADING'?'text-muted-foreground dark:text-muted-foreground':'text-warning-subtle-foreground dark:text-warning-subtle-foreground';

  return <div ref={panelRef} className="mt-7 border-t border-border pt-6 dark:border-border">
    ...
    <Card className="p-4">
      <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><label className="block flex-1 text-xs font-semibold text-muted-foreground dark:text-muted-foreground">Camera Device<Select className="mt-1.5" value={sourceKey} disabled={camera.status==='READY'} onChange={event=>setSourceKey(event.target.value)}>{!camera.devices.length&&<option value="">No camera detected</option>}{camera.devices.map(device=><option key={device.id} value={device.id}>{device.label}</option>)}</Select></label><div className="flex flex-wrap gap-2">{camera.status==='READY'?<Btn variant="secondary" onClick={camera.stop}>Stop Camera</Btn>:<Btn onClick={start}>Start Camera</Btn>}<Btn disabled={!canCapture} loading={loadingCapture} onClick={()=>captureWhiteboard('MANUAL')}>{loadingCapture?'Processing Capture…':<><Camera size={15}/> Capture Whiteboard</>}</Btn></div></div>
      <div className="mb-3 flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl bg-surface-subtle/90 px-3 py-2.5 text-xs font-semibold dark:bg-card/[.04]"><span className={`flex items-center gap-1.5 ${camera.status==='READY'?'text-success-subtle-foreground dark:text-success-subtle-foreground':camera.status==='ERROR'?'text-destructive-subtle-foreground dark:text-destructive-subtle-foreground':'text-muted-foreground dark:text-muted-foreground'}`}><Camera size={14}/><span className="h-1.5 w-1.5 rounded-full bg-current"/>Camera {camera.status==='READY'?'Ready':camera.status}</span><span className={`flex items-center gap-1.5 ${calibrationClass}`}><Check size={14}/>{calibrationLabel}</span><LessonLightingStatus lesson={lesson} onStatusChange={setLightingReadiness}/></div>
      <div className="mb-4 flex flex-wrap items-center gap-2 text-xs"><span className={`rounded-full px-2.5 py-1 font-bold ${autoMonitoring?'bg-success-subtle text-success-subtle-foreground dark:bg-success-subtle dark:text-success-subtle-foreground':'bg-surface-elevated text-muted-foreground dark:bg-card/10 dark:text-muted-foreground'}`}>Auto Capture: {calibrationStatus==='LOADING'?'WAITING':autoMonitoring?'ON':'OFF'} • {autoModeLabel}</span><span className="text-muted-foreground">{autoStatus}</span></div>
      ...
      {visiblePreviewMode==='LATEST'&&latestCapture&&<div className="isolate relative aspect-video w-full overflow-hidden rounded-2xl border border-border bg-slate-950 shadow-inner dark:border-border">{latestLocalPreview?.captureId===latestCapture.id?<img src={latestLocalPreview.url} alt="Latest corrected whiteboard capture" className="h-full w-full object-contain"/>:<ProtectedCaptureImage url={versionedImageUrl(latestCapture)} alt="Latest corrected whiteboard capture" className="h-full w-full object-contain"/>}<span className="absolute left-3 top-3 rounded-full bg-slate-950/75 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white">Latest Corrected Capture</span><span className="absolute right-3 top-3 rounded-full bg-slate-950/75 px-2.5 py-1 text-[10px] font-semibold text-white">{new Date(latestCapture.capturedAt).toLocaleString('en-PH')}</span></div>}
      {visiblePreviewMode==='LIVE'&&<div className="isolate relative aspect-video w-full overflow-hidden rounded-2xl border border-border bg-slate-950 shadow-inner dark:border-border"><canvas ref={correctedCanvasRef} className={`h-full w-full object-contain ${correctedReady?'opacity-100':'opacity-0'}`} aria-label={mode==='SIMULATED'?'Perspective-corrected demo whiteboard preview':'Live perspective-corrected whiteboard preview'}/><span className="absolute left-3 top-3 rounded-full bg-slate-950/75 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white">{mode==='SIMULATED'?'Demo Corrected Whiteboard':'Live Corrected Whiteboard'}</span>{camera.status!=='READY'&&<div className="absolute inset-0 grid place-items-center px-6 text-center text-sm text-slate-300">{mode==='SIMULATED'?'Start the demo whiteboard preview.':'Start the camera to display the corrected whiteboard.'}</div>}{camera.status==='READY'&&calibrationStatus==='LOADING'&&<div className="absolute inset-0 grid place-items-center px-6 text-center text-sm text-slate-300">Loading camera calibration…</div>}{camera.status==='READY'&&calibrationStatus==='NOT_CALIBRATED'&&<div className="absolute inset-0 grid place-items-center px-6 text-center text-sm font-medium text-warning-subtle-foreground">Calibration required for this camera.</div>}{camera.status==='READY'&&calibrationStatus==='INVALID'&&<div className="absolute inset-0 grid place-items-center px-6 text-center text-sm font-medium text-destructive-subtle-foreground">The saved calibration is invalid.</div>}{camera.status==='READY'&&calibrationStatus==='CALIBRATED'&&calibration&&!correctedReady&&<div className="absolute inset-0 grid place-items-center px-6 text-center text-sm text-slate-300">Preparing corrected whiteboard view…</div>}</div>}
      ...
      {previewError&&<Alert type="error" className="mt-3">{previewError}</Alert>}{lesson.status==='PAUSED'&&!settings.captureWhilePaused&&<Alert type="warning" label="Capture paused" className="mb-0 mt-3">Capture is disabled while the lesson is paused. The camera session remains available for resume.</Alert>}{calibrationStatus==='LOADING'&&<p className="mt-3 text-xs font-medium text-muted-foreground dark:text-muted-foreground">Loading camera calibration…</p>}{['NOT_CALIBRATED','INVALID'].includes(calibrationStatus)&&<Alert type="warning" label="Calibration required" className="mb-0 mt-3" actions={<Btn variant="secondary" size="sm" onClick={openCalibration}>Open Hardware Settings</Btn>}>{calibrationStatus==='INVALID'?'The saved camera calibration is invalid.':'Camera calibration is required.'}</Alert>}<p className="mt-3 text-xs text-muted-foreground">Last capture: {latestCapture?new Date(latestCapture.capturedAt).toLocaleString('en-PH'):'No captures yet'}</p>
    </Card>
    {captures.length>0&&<div className="mt-5"><h3 className="mb-3 text-sm font-semibold text-foreground dark:text-foreground">Recent Lesson Captures</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{captures.slice(0,4).map(capture=><Card key={capture.id} className="overflow-hidden"><ProtectedCaptureImage url={capture.correctedUrl||capture.originalUrl} alt={`Whiteboard captured ${new Date(capture.capturedAt).toLocaleString()}`} className="aspect-video w-full bg-slate-950 object-contain"/><div className="px-3 py-2 text-xs text-muted-foreground">{new Date(capture.capturedAt).toLocaleString('en-PH')}</div></Card>)}</div></div>}
    {autoToast&&<Alert type="success" label="Capture saved" title="Whiteboard automatically captured" className="!mb-0 fixed bottom-5 right-5 z-40 w-[min(92vw,420px)] shadow-glass" actions={<><Btn variant="ghost" size="sm" onClick={()=>{setPreviewMode('LATEST');panelRef.current?.scrollIntoView({behavior:'smooth',block:'start'});}}>View</Btn><Btn variant="danger" size="sm" onClick={undoAutoCapture}><Trash size={13}/> Undo</Btn></>}>Saved at {autoToast.time.toLocaleTimeString('en-PH')}.</Alert>}
  </div>;
}
