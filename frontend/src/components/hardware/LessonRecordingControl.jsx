...
export default function LessonRecordingControl({recording,onDeleted,onError}){
  ...
  if(!recording)return <span className="text-xs text-muted-foreground">No audio</span>;
  ...
  return <div className="min-w-48"><div className="flex flex-wrap items-center gap-1.5"><span className="mr-1 text-xs font-semibold text-muted-foreground dark:text-muted-foreground">{fmt(recording.durationMs)}</span><Btn variant="secondary" size="sm" onClick={()=>setOpen(value=>!value)}><Play size={13}/> {open?'Hide':'Play'}</Btn><Btn variant="danger" size="sm" aria-label="Delete lesson audio recording" onClick={()=>setConfirm(true)}><Trash size={13}/></Btn></div>{open&&<div className="mt-2 w-72 max-w-[70vw]"><ProtectedAudioPlayer url={recording.audioUrl}/><p className="mt-1 text-[11px] text-muted-foreground">Recorded {new Date(recording.recordedAt).toLocaleDateString('en-PH',{month:'short',day:'numeric',year:'numeric'})}</p></div>}{confirm&&<ConfirmModal title="Delete this lesson recording?" body="This audio recording will be permanently removed. This action cannot be undone." confirmLabel="Delete Recording" confirmVariant="danger" loading={deleting} onConfirm={remove} onCancel={()=>setConfirm(false)}/>}</div>;
}
