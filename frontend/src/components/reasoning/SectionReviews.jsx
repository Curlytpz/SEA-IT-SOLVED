...
export default function SectionReviews({ sectionId, data, loading, error, onRetry }) {
  ...
  return <section aria-labelledby="section-reviews-title">
    ...
    {noSubmissions?<div className="mt-5 rounded-xl border border-slate-200 p-5 dark:border-slate-700"><p className="font-semibold">No quiz submissions yet.</p><p className="mt-1 text-sm text-slate-500">Published quizzes and their attempts will appear here when students submit.</p></div>:
      <div className="mt-5 space-y-3">{visible.map(quiz=><Card key={quiz.quizId} className="p-4 sm:p-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-slate-900 dark:text-white"><GeneratedContent markdown={quiz.title} quizText inline/></h3><span className="text-xs font-semibold text-slate-500">{quiz.status}</span></div><p className="mt-1 text-sm text-slate-500">Lesson: <Link className="font-semibold text-primary-subtle-foreground hover:underline" to={`/instructor/lessons/${quiz.lessonId}/review?view=workspace`} state={reviewsOriginState}><GeneratedContent markdown={quiz.lessonTitle} inline/></Link></p><p className="mt-3 text-sm text-slate-700 dark:text-slate-300">{quiz.submittedAttempts} submitted {quiz.submittedAttempts===1?'attempt':'attempts'} · {quiz.pendingResponses?quiz.pendingResponses+' awaiting review':'All reviewed'} · {quiz.problemSolvingQuestions} Problem Solving {quiz.problemSolvingQuestions===1?'question':'questions'}</p></div><Link to={`/instructor/quizzes/${quiz.quizId}/attempts`} state={{...reviewsOriginState,from:reviewsReturnTo}} className="shrink-0"><Btn>{quiz.pendingResponses?'Review Attempts':'View Attempts'}</Btn></Link></div></Card>)}
