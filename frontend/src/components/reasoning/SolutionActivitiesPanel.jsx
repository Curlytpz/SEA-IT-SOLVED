  ...
  return <Dialog open onOpenChange={open => { if (!open && !busy) onClose(); }}><DialogContent className="max-w-[calc(100%-1rem)] sm:max-w-6xl">
    ...
    <div className="grid min-w-0 gap-4 lg:grid-cols-3">
      ...
      <section className="min-w-0 space-y-4 rounded-xl border border-slate-200 p-3 dark:border-slate-700"><div><h3 className="flex items-center gap-2 font-semibold"><BrainCircuit size={17}/>AI Advisory Review</h3><p className="mt-1 text-xs text-slate-500">AI analysis is advisory. Final assessment and grading remain under instructor control.</p></div>
        {busy === 'analyze' ? <ReviewProgress/> : submission.aiReview ? <div className="space-y-3 text-sm"><Badge status={submission.aiReview.assessment}/><p>{submission.aiReview.summary}</p>{submission.aiReview.strengths.length > 0 && <div><strong>Strengths</strong><ul className="mt-1 list-disc space-y-1 pl-5">{submission.aiReview.strengths.map((item, index) => <li key={index}>{item}</li>)}</ul></div>}{submission.aiReview.possibleErrors.length > 0 && <div><strong>Possible errors to verify</strong><ul className="mt-1 space-y-2">{submission.aiReview.possibleErrors.map((item, index) => <li key={index} className="rounded-lg bg-amber-50 p-2 dark:bg-amber-500/10"><b>{item.step || `Step ${index + 1}`}</b><br/>{item.issue}{item.suggestion && <><br/><span className="text-slate-500">{item.suggestion}</span></>}</li>)}</ul></div>}{submission.aiReview.suggestedFeedback && <div><strong>Suggested feedback</strong><p className="mt-1">{submission.aiReview.suggestedFeedback}</p></div>}<Btn variant="secondary" onClick={onAnalyze}>{analysisFailed ? 'Retry Analysis' : 'Analyze Again'}</Btn></div> : <Btn disabled={submission.recognitionStatus !== 'READY'} onClick={onAnalyze}><BrainCircuit size={16}/>{analysisFailed ? 'Retry Analysis' : 'Analyze with AI'}</Btn>}
