...
export default function StudentSolutionActivities({ lessonId }) {
  ...
  return <section className="mt-7 space-y-4" aria-labelledby="student-solution-activities-title">
    <div><h2 id="student-solution-activities-title" className="text-lg font-bold text-foreground dark:text-foreground">Solution Activities</h2><p className="text-sm text-muted-foreground">Upload your handwritten work for instructor review.</p></div>
    ...
    {activities.length ? <div className="grid gap-4 lg:grid-cols-2">{activities.map(activity => {
      ...
      return <Card key={activity.id} className="min-w-0 p-5"><div className="flex items-start justify-between gap-3"><div><Badge status={activity.status}/><h3 className="mt-2 font-semibold text-foreground dark:text-foreground"><GeneratedContent markdown={activity.title} audience="student" inline/></h3></div><span className="shrink-0 text-sm font-bold text-primary-subtle-foreground">{activity.maxPoints} pts</span></div><div className="mt-3 break-words text-sm text-foreground dark:text-foreground"><GeneratedContent markdown={activity.problemText}/></div>{activity.instructions && <div className="mt-3 rounded-lg bg-surface-subtle p-3 text-sm text-muted-foreground dark:bg-surface-elevated dark:text-muted-foreground"><strong className="block text-xs uppercase tracking-wide">Instructions</strong><GeneratedContent markdown={activity.instructions} audience="student" mathFallback/></div>}<p className="mt-3 inline-flex items-center gap-1 text-xs text-muted-foreground"><CalendarDays size={14}/>{dateText(activity.dueAt)}</p>
        {submission ? <div className="mt-4 space-y-3 border-t border-border pt-4 dark:border-border"><div className="flex flex-wrap items-center gap-2"><Badge status={submission.status}/><span className="text-sm font-medium text-muted-foreground dark:text-muted-foreground">{submission.status === 'GRADED' ? 'Instructor feedback released' : 'Submitted • Awaiting Instructor Review'}</span></div><div className="overflow-hidden rounded-lg bg-slate-950"><ProtectedCaptureImage url={submission.imageUrl} alt="Your submitted handwritten solution" className="max-h-48 w-full object-contain"/></div>{submission.feedbackReleasedAt && <Alert type="success" label="Grade released" title={`Score: ${submission.finalScore} / ${activity.maxPoints}`} className="mb-0"><strong>Instructor feedback:</strong><GeneratedContent markdown={submission.instructorFeedback || 'No written feedback provided.'} audience="student" mathFallback/></Alert>}{activity.acceptingSubmissions && submission.status !== 'GRADED' && <Btn variant="secondary" className="w-full" onClick={() => { setSelected(activity); setError(''); }}><FileImage size={16}/>Replace Solution Image</Btn>}</div> : <Btn className="mt-4 w-full" disabled={!activity.acceptingSubmissions} onClick={() => { setSelected(activity); setError(''); }}><Upload size={16}/>{activity.acceptingSubmissions ? 'Submit Solution' : 'Submissions Closed'}</Btn>}
      </Card>;
    ...
    {selected && <Dialog open onOpenChange={open => { if (!open) close(); }}><DialogContent className="sm:max-w-xl"><DialogHeader><DialogTitle>Submit Solution</DialogTitle><DialogDescription>{selected.title} • Upload one clear image of your handwritten work.</DialogDescription></DialogHeader>{error && <Alert>{error}</Alert>}<label className="flex min-h-36 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border p-5 text-center hover:border-primary/40 dark:border-border"><Upload size={24} className="text-primary-subtle-foreground"/><strong>{file ? 'Replace Image' : 'Choose Solution Image'}</strong><span className="text-xs text-muted-foreground">JPEG, PNG, or WebP • maximum 8 MB</span><input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={busy} onChange={event => chooseFile(event.target.files?.[0])}/></label>{preview && <img src={preview} alt="Selected solution preview" className="max-h-[45vh] w-full rounded-xl bg-slate-950 object-contain"/>}<DialogFooter><Btn variant="ghost" disabled={busy} onClick={close}>Cancel</Btn><Btn loading={busy} loadingText="Submitting and recognizing…" disabled={!file} onClick={submit}>Submit Solution</Btn></DialogFooter></DialogContent></Dialog>}
  </section>;
}
