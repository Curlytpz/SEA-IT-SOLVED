...
export default function StudentSolutionActivities({ lessonId }) {
  ...
  return <section className="mt-7 space-y-4" aria-labelledby="student-solution-activities-title">
    ...
    {activities.length ? <div className="grid gap-4 lg:grid-cols-2">{activities.map(activity => {
      ...
      return <Card key={activity.id} className="min-w-0 p-5"><div className="flex items-start justify-between gap-3"><div><Badge status={activity.status}/><h3 className="mt-2 font-semibold text-slate-900 dark:text-white">{activity.title}</h3></div><span className="shrink-0 text-sm font-bold text-primary-subtle-foreground">{activity.maxPoints} pts</span></div><div className="mt-3 break-words text-sm text-slate-700 dark:text-slate-200"><GeneratedContent markdown={activity.problemText}/></div>{activity.instructions && <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300"><strong className="block text-xs uppercase tracking-wide">Instructions</strong>{activity.instructions}</div>}<p className="mt-3 inline-flex items-center gap-1 text-xs text-slate-500"><CalendarDays size={14}/>{dateText(activity.dueAt)}</p>
        {submission ? <div className="mt-4 space-y-3 border-t border-slate-200 pt-4 dark:border-slate-700"><div className="flex flex-wrap items-center gap-2"><Badge status={submission.status}/><span className="text-sm font-medium text-slate-600 dark:text-slate-300">{submission.status === 'GRADED' ? 'Instructor feedback released' : 'Submitted • Awaiting Instructor Review'}</span></div><div className="overflow-hidden rounded-lg bg-slate-950"><ProtectedCaptureImage url={submission.imageUrl} alt="Your submitted handwritten solution" className="max-h-48 w-full object-contain"/></div>{submission.feedbackReleasedAt && <Alert type="success" label="Grade released" title={`Score: ${submission.finalScore} / ${activity.maxPoints}`} className="mb-0"><strong>Instructor feedback:</strong> {submission.instructorFeedback || 'No written feedback provided.'}</Alert>}{activity.acceptingSubmissions && submission.status !== 'GRADED' && <Btn variant="secondary" className="w-full" onClick={() => { setSelected(activity); setError(''); }}><FileImage size={16}/>Replace Solution Image</Btn>}</div> : <Btn className="mt-4 w-full" disabled={!activity.acceptingSubmissions} onClick={() => { setSelected(activity); setError(''); }}><Upload size={16}/>{activity.acceptingSubmissions ? 'Submit Solution' : 'Submissions Closed'}</Btn>}
      </Card>;
