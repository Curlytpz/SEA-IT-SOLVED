...
export default function StudentSolutionActivities({ lessonId }) {
  ...
  return <section className="mt-7 space-y-4" aria-labelledby="student-solution-activities-title">
    ...
    {activities.length ? <div className="grid gap-4 lg:grid-cols-2">{activities.map(activity => {
      ...
      return <Card key={activity.id} className="min-w-0 p-5"><div className="flex items-start justify-between gap-3"><div><Badge status={activity.status}/><h3 className="mt-2 font-semibold text-slate-900 dark:text-white"><GeneratedContent markdown={activity.title} audience="student" inline/></h3></div><span className="shrink-0 text-sm font-bold text-primary-subtle-foreground">{activity.maxPoints} pts</span></div><div className="mt-3 break-words text-sm text-slate-700 dark:text-slate-200"><GeneratedContent markdown={activity.problemText}/></div>{activity.instructions && <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300"><strong className="block text-xs uppercase tracking-wide">Instructions</strong><GeneratedContent markdown={activity.instructions} audience="student" mathFallback/></div>}<p className="mt-3 inline-flex items-center gap-1 text-xs text-slate-500"><CalendarDays size={14}/>{dateText(activity.dueAt)}</p>
