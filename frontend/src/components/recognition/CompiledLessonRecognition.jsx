...
export default function CompiledLessonRecognition({ recognition, selectedPage, onSelectPage }) {
  ...
  return <section className="mt-5" aria-labelledby="compiled-recognition-title">
    <Card className="p-4 sm:p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 id="compiled-recognition-title" className="text-lg font-bold text-foreground dark:text-foreground">Compiled Lesson Recognition</h2><p className="mt-1 text-xs leading-5 text-muted-foreground dark:text-muted-foreground">All whiteboard pages are analyzed together while their original order stays intact.</p></div><RecognitionStatusBadge status={status}/></div>
      ...
      {result&&<div className="mt-5">
        ...
        <div className="max-h-[680px] space-y-4 overflow-y-auto overscroll-contain pr-1 sm:pr-2">{pages.map(page=><article key={page.captureId||page.pageNumber} id={`recognized-page-${page.pageNumber}`} className={`rounded-xl border p-3 sm:p-4 ${selectedPage===page.pageNumber?'border-primary/40 bg-primary-subtle':'border-border bg-card/45'}`}>
          <h3 className="mb-3 text-sm font-bold text-foreground dark:text-foreground">Page {page.pageNumber}</h3>
          <div className="space-y-2">{page.blocks?.length?page.blocks.map((block,index)=><div key={`${block.order}-${index}`} className="rounded-xl bg-card/70 p-3 dark:bg-card/[.045]">{block.type==='math'?<MathExpression latex={block.latex}/>:<GeneratedContent markdown={block.text} className="recognition-math-content text-sm leading-6 text-foreground dark:text-foreground" reviewIndicator mathFallback/>}{block.uncertain&&<div className="mt-2"><UncertaintyBadge reason={block.uncertaintyReason}/></div>}</div>):<GeneratedContent markdown={page.plainText||'No visible content recognized.'} className="recognition-math-content text-sm leading-6 text-foreground dark:text-foreground" reviewIndicator mathFallback/>}</div>
          {page.warnings?.map((warning,index)=><p key={index} className="mt-2 text-xs text-warning-subtle-foreground dark:text-warning-subtle-foreground">{warning}</p>)}
        </article>)}</div>
