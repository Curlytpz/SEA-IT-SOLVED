...
const styles = {
  ...
  RETRYING: 'bg-info-subtle text-info ring-info/20',
...
export default function RecognitionStatusBadge({ status = 'NOT_STARTED' }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ring-inset ${styles[normalizedStatus] || styles.NOT_STARTED}`}>
      {['PROCESSING', 'RETRYING'].includes(normalizedStatus) && <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-current" aria-hidden="true" />}
