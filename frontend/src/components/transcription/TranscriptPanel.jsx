...
export default function TranscriptPanel({ lessonId, data, loading, error, onRefresh }) {
  ...
  return <section aria-labelledby="audio-transcript-heading">
    <Card className="overflow-hidden">
      <div className="border-b border-border/80 p-4 dark:border-border sm:p-5">
      ...
      <div className="p-4 sm:p-5">
        ...
        {loading&&!data?<LoadingState text="Loading lesson audio…"/>:!recording?<div className="py-8 text-center"><div className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-xl bg-info-subtle text-info"><Mic size={21}/></div><p className="font-semibold text-foreground">No lesson audio was saved</p><p className="mt-1 text-sm text-muted-foreground">Whiteboard compilation remains available.</p></div>:<>
          <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-semibold text-muted-foreground">{Math.round(recording.durationMs/1000)} sec • {(recording.fileSize/1024/1024).toFixed(1)} MB</p><TranscriptStatusBadge status={status}/></div>
          ...
          {['PENDING','PROCESSING'].includes(status)&&!result&&<p className="mt-5 text-sm text-muted-foreground">Waiting for the transcription worker…</p>}
          {status==='FAILED'&&!result&&<p className="mt-5 text-sm text-destructive-subtle-foreground dark:text-destructive-subtle-foreground">{failureText(transcription)}</p>}
          {result&&<div className="mt-5"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h3 className="text-sm font-bold text-foreground dark:text-foreground">Transcript timeline</h3>{transcription.language&&<span className="text-[11px] text-muted-foreground">{transcription.language}</span>}</div>{result.timeline?.timingWarning&&<Alert type="warning">{result.timeline.timingWarning}</Alert>}<div className="max-h-[min(62vh,720px)] overflow-y-auto overscroll-contain pr-1"><TranscriptTimeline segments={transcription.segments||result.segments} onSeek={seek}/></div></div>}
        </>}
      </div>
    </Card>
  </section>;
