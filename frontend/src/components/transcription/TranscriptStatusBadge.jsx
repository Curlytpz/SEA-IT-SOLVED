...
const labels = { NOT_STARTED: 'Not processed', PENDING: 'Waiting', PROCESSING: 'Processing', REVIEW_REQUIRED: 'Ready for review', FAILED: 'Needs retry' };

export default function TranscriptStatusBadge({ status = 'NOT_STARTED' }) {
  return <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ring-inset ${styles[status] || styles.NOT_STARTED}`}>
