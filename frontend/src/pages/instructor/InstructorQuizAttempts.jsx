...
import { Alert, BackButton, Btn, Card, LoadingState, Select } from '../../components/ui';
...
export default function InstructorQuizAttempts(){
  ...
  return <DashboardLayout>
    <BackButton replace to={backTo} state={backToSection?undefined:lessonBackState}>{backToSection?'Back to section reviews':'Back to lesson workspace'}</BackButton>
    ...
    {!data?!error&&<LoadingState text="Loading quiz attempts…"/>:<>
      <h1 className="text-2xl font-bold"><GeneratedContent markdown={data.quiz.title} quizText inline/></h1><p className="mt-2 text-sm text-muted-foreground">{data.attempts.length} attempts · {data.attempts.reduce((sum,a)=>sum+a.pending,0)} responses awaiting review</p>
      <div className="mt-6 grid min-w-0 gap-5 xl:grid-cols-[19rem_minmax(0,1fr)]">
        ...
        <div className="mt-4 space-y-2">{data.attempts.filter(a=>filter==='ALL'||a.status===filter).map(a=><button key={a.id} type="button" aria-pressed={a.id===selected} onClick={()=>setSelected(a.id)} className={`w-full rounded-lg border p-3 text-left text-sm ${a.id===selected?'border-primary/40 bg-primary-subtle':'border-border'}`}>
          ...
          <span className="block text-xs text-muted-foreground">{a.status==='IN_PROGRESS'?'Not yet submitted':`${a.objectiveScore} / ${a.objectiveMax} objective · ${a.pending} need review`}</span>
          {a.submittedAt&&<time className="mt-1 block text-xs text-muted-foreground">{new Date(a.submittedAt).toLocaleString()}</time>}
        </button>)}{!data.attempts.length&&<p className="text-sm text-muted-foreground">No attempts yet.</p>}</div></Card>
